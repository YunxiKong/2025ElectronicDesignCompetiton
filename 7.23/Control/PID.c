...
float LocationL, LocationR, SpeedLOUT, SpeedROUT;

//static int Basic_Speetarget_Positiond = 0;    //基础速度，在这里修改速度
...
PID_t OuterL = {				//外环PID结构体变量，定义的时候同时给部分成员赋初值
	.Kp = 0.1,					//比例项权重
	.Ki = 0,					//积分项权重
	.Kd = 0,					//微分项权重
};

PID_t OuterR = {				//外环PID结构体变量，定义的时候同时给部分成员赋初值
	.Kp = 0.1,					//比例项权重
	.Ki = 0,					//积分项权重
	.Kd = 0,					//微分项权重
};
...
//			LocationR += SpeedR;
//			
//			LocationL += Encoder_GetL()/5.2;			
//			LocationR += Encoder_GetR()/5.2;
//		
//			OuterL.Actual = LocationL;
//			OuterR.Actual = LocationR;
//			
//			OuterL.Target = target_Position;
//			OuterR.Target = target_Position;
//			
//			Outer_PID_Update(&OuterL);
//			Outer_PID_Update(&OuterR);
//			
//			MotorL_SetSpeed((int)OuterL.Out);
//			MotorR_SetSpeed((int)OuterR.Out);
//		}	 
//}
